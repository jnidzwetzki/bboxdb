...
public class SocketImporter implements Runnable {
	...
	public SocketImporter(final int port, final String connectionPoint, final String clustername, final String table,
			final TupleBuilder tupleFactory, final QueryEnlargement enlargement,
	...
	public static void main(final String[] args)
		...
		if (args.length != 7) {
			System.err.println(
					"Usage: <Class> <Port> <Connection Endpoint> <Clustername> <Table> <Format> <Enlargement> <Write-To-Disk>");
			System.exit(-1);
		}
		...
		if ("dynamic".equals(enlargement)) {
			...
			final ContinuousQueryEnlargementRegisterer continuousQueryRegisterer = ContinuousQueryEnlargementRegisterer
					.getInstanceFor(tupleStoreName);
			...
			final double enlargementFactor = MathUtil.tryParseDoubleOrExit(enlargement,
					() -> "Unable to parse enlargement: " + enlargement);
		...
		final EnumSet<DistributionRegionHandlingFlag> insertOptions = EnumSet
				.noneOf(DistributionRegionHandlingFlag.class);
		;
