...
public class SpatialIndexFactoryHelper {
	
	/**
	 * The cached instance of the factory
	 */
	protected static SpatialIndexFactory cachedInstance = null;
	
	/**
	 * The Logger
	 */
	private final static Logger logger = LoggerFactory.getLogger(SpatialIndexFactoryHelper.class);

	/**
	 * Get an instance of the configured factory
	 * @return
	 */
	public static SpatialIndexFactory getFactoryInstance() {
		...
		try {
			...
			if(! (factoryObject instanceof SpatialIndexFactory)) {
				throw new ClassNotFoundException(factoryClass + " is not a instance of SpatialIndexFactory");
			}
			
			cachedInstance = (SpatialIndexFactory) factoryObject;
